...
- **BF-888 driver** implementing the programming protocol.
...
- Read a BF-888 codeplug from a connected radio.
- Upload a codeplug to a BF-888.
...
```
HARDWARE_TESTS=1 SERIAL_PORT=/dev/tty.usbserial-XXXX npm run test:hw
```

Enable actual programming (writes):

```
HARDWARE_TESTS=1 SERIAL_PORT=/dev/tty.usbserial-XXXX ENABLE_PROGRAMMING=1 npm run test:hw
```
